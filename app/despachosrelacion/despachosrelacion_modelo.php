...
class DespachosRelacion extends Conectar{
    ...
    public function get_productos_devueltos_de_un_despacho($correlativo) {
        ...
        $sql = "SELECT DISTINCT CodItem, Descrip,
                    ...
                    EsUnid = 0 AND TipoFac = 'B' AND OTipo = 'A' AND (ONumero IN ( SELECT Numerod FROM [appweb-sucre_SUCRE].dbo.Despachos_Det WHERE ID_Correlativo = ? ))), 0)
                    AS BULTOS,
                    COALESCE((SELECT SUM(Cantidad) FROM SAITEMFAC WHERE CodItem = SAPROD.CodProd AND
                    EsUnid = 1 AND TipoFac = 'B' AND OTipo = 'A' AND (ONumero IN ( SELECT Numerod FROM [appweb-sucre_SUCRE].dbo.Despachos_Det WHERE ID_Correlativo = ? ))), 0)
                    AS PAQUETES,
                    CantEmpaq,
                    EsEmpaque,
                    saprod.Tara AS tara,
                    ...
                    TipoFac = 'B' AND OTipo = 'A' AND (ONumero IN ( SELECT Numerod FROM [appweb-sucre_SUCRE].dbo.Despachos_Det WHERE ID_Correlativo = ? )) ORDER BY SAITEMFAC.CodItem";
    ...
    public function get_facturas_devueltas_de_un_despacho($correlativo) {
        ...
        $sql = "SELECT DISTINCT ONumero
				...
				WHERE TipoFac = 'B' AND OTipo = 'A'  AND (ONumero IN ( SELECT Numerod FROM [appweb-sucre_SUCRE].dbo.Despachos_Det WHERE ID_Correlativo = ? )) 
