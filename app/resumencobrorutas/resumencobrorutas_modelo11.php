            ...
            if($tipo === 'B' and $ruta === 'Todos'){
              ...
              GROUP BY EDV order by EDV asc";
                ...
                if($tipo === 'D' and $ruta === 'Todos'){
                    ...
                    c.CodVend EDV,
                    case when DATEDIFF(day, cxc.FechaE, p.FechaE) >= 0 and DATEDIFF(day, cxc.FechaE, p.FechaE) <= 7 then
                    p.Monto - isnull((select dev.Monto from [SUCRE_D].[dbo].SAACXC as dev where dev.NroUnico = p.NroPpal and dev.TipoCxc in ('31')),0) 
                    else 0 end De_0_a_7_Dias,
                    ...
                    p.Monto - isnull((select dev.Monto from [SUCRE_D].[dbo].SAACXC as dev where dev.NroUnico = p.NroPpal and dev.TipoCxc in ('31')),0) Total
                    
                    from [SUCRE_D].[dbo].SAPAGCXC as p 
                    inner join [SUCRE_D].[dbo].SAACXC as c on p.NroPpal = c.NroUnico 
                    inner join [SUCRE_D].[dbo].SACLIE as cl on c.CodClie = cl.CodClie
                    left join [SUCRE_D].[dbo].SAACXC as cxc on cxc.NumeroD = p.NumeroD and cxc.TipoCxc in ('10','20')
