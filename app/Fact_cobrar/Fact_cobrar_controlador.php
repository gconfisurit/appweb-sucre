...
switch ($_GET["op"]) {
    ...
    $datos = $facturas->getfacturasporcobrar( $_POST["vendedor"]);
    ...
    foreach ($datos as $row) {
        ...
        $sub_array[] = '<div class="col text-center"><a href="../Fact_cobrar/detalles.php?vendedor='.$vendedor.'&data='.$data1.'""> '.$Total_0_a_7_Dias.'</div>';
        $sub_array[] = '<div class="col text-center"><a href="../Fact_cobrar/detalles.php?vendedor='.$vendedor.'&data='.$data2.'""> '.$Total_8_a_15_Dias.'</div>';
        $sub_array[] = '<div class="col text-center"><a href="../Fact_cobrar/detalles.php?vendedor='.$vendedor.'&data='.$data3.'""> '.$Total_16_a_40_Dias.'</div>';
        $sub_array[] = '<div class="col text-center"><a href="../Fact_cobrar/detalles.php?vendedor='.$vendedor.'&data='.$data4.'""> '.$Total_Mayor_a_40_Dias.'</div>';
        $sub_array[] = '<div class="col text-center"><a href="../Fact_cobrar/detalles.php?vendedor='.$vendedor.'&data='.$data5.'""> '.$SubTotal.'</div>';
    ...
    case "detalle_de_facturas":
       ...
       // var_dump($dataop);
        ...
        $datos = $facturas->getdetallesfacturasporcobrar( $vendedor, $dataop);
$suma_monto=0;
$suma_montodolar=0;
        foreach ($datos as $row) {
            //DECLARAMOS UN SUB ARRAY Y LO LLENAMOS POR CADA REGISTRO EXISTENTE.
